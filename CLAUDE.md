...
- **Entrega de mensajes al terminal (desde 2026-09-07, `1308d35b`):** "listo" = banner + 400ms y "entregado" = bytes escritos NO garantizan recepción; un CLI arrancando pierde las teclas (Stanley4, Holly). Ahora el drain confirma con el hook `UserPromptSubmit` y reintenta; en `dev.log` del renderer no hay traza, pero el main loggea `[worker-wake] holding/nudging`. La TUI de Claude NUNCA queda en silencio (`pty quiet 0s` idle): no usar silencio del PTY como señal.
- **OpenWolf en god (desde 2026-09-07):** hooks en `<harnessHome>/.claude/settings.json` (cwd de god; ningún worker lo usa), `.wolf` en `hive/agents/god/.wolf`, comandos envueltos `env -u CLAUDE_PROJECT_DIR node …` porque Claude Code exporta `CLAUDE_PROJECT_DIR=<cwd>` a los hooks y OpenWolf lo prioriza (sin el envoltorio salen sin hacer nada). `agents/<id>/settings.json` lo REGENERA el harness en cada spawn: no sirve para persistir. Validar con `hive/agents/god/.wolf/hooks/_heartbeat.json` (last_ok por hook), nunca por "el archivo existe". Rollback: `rm <harnessHome>/.claude/settings.json && rm -rf hive/agents/god/.wolf`.
...
- **PRs ABIERTOS: #454** `fix/human-ask-visibility` (`798b7881`), **#455** `fix/worker-wake-stall` (`845dd0e6` v3: stall salta `announced`, actividad = span o muestra CON tokens) y **fix/prompt-delivery-ack** (`1308d35b`, PR abierto 2026-09-07: entrega confirmada por `UserPromptSubmit`), base `origin/main` `3e4f9f62`; Build/Typecheck/Evidence verdes; worktrees `../munder-difflin-pr-human-ask` y `../munder-difflin-pr-worker-wake`. Ambos ya integrados en la rama de trabajo (`1721e354`): al mergear upstream, `git diff origin/main -- src test` dejará de estar vacío hasta que entren.
